function varargout=est_cryosat_dzdt_2mode_MOA_edit(varargin)
...
switch lower(region_name)
    ...
    case('test')
        if 0
         ...
         if 1
            % PIG GL
            XG= -1.6000e+06 - 2.5000e+05i ;
            region_params.E_RMS_d2z0dx2=2e-7;
            region_params.E_RMS_d3zdx2dt=6e-9;
            region_params.E_RMS_d2zdt2=0.2;
            params.skip_MOA_curv=true;
        end
...
out_dir='/Volumes/ice1/ben/Cryosat/fits_C/Helm_picks/';
...
if ~isfield(params,'skip_MOA_curv')
...
%      end
    ...
    [MOA_mask, MOA, d_MOA]=filter_z0_with_MOA(D, z0, grids, MOA, d_MOA);
    if true
        figure; imagesc(grids.z0.ctrs{2}, grids.z0.ctrs{1}, gradient(z0)); hold on;
        plot(D.x(good), D.y(good),'r.','markersize', 1);
        plot(D.x(good & ~MOA_mask), D.y(good&~MOA_mask),'b.','markersize', 1)
        drawnow
    end
    %disp('NO MOA MASK APPLIED');
    good(~MOA_mask)=false;
