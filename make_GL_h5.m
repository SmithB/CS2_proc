function  varargout=make_GL_h5(varargin)
...
while ~feof(fid_list)
    ...
    count=count+1;
    queue_list{count}=sprintf( 'make_GL_h5(''run_bin'', ''%s'', ''%s'', ''%s'', %d);', master_index_file, out_dir, fname, dx0);
end
fclose(fid_list);

if nargout==0
    fid_queue=fopen(sprintf('queue_for_GL%s.txt',mode),'w');
...
fprintf(1,'make_GL_h5: working on %s\n', out_file);
